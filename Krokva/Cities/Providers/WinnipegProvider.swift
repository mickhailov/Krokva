...
final class WinnipegProvider: SocrataProvider, CityDataProvider {
    ...
    init() {
        super.init(domain: KrokvaBackend.host, scheme: KrokvaBackend.scheme)
    }
    ...
    // Shared by domain extensions (e.g. WinnipegProvider+Schools), so internal not private.
    func streetCore(_ name: String) -> String {
    ...
    private func fetchPublicNotices(property: PropertyAssessment) async -> [PublicNotice] {
        guard let dataset = datasets.publicNotices,
              let subject = property.coordinate else { return [] }
        let rows = (try? await fetch(dataset, queryItems: [
            URLQueryItem(name: "$select", value: "notice_type,community,notice_id,address,description,conditions,approval_type,decision,meeting_date,ward,neighbourhood,roll_number,appeal_date,report_plan_url,plain_language,point"),
        ...
        return rows.compactMap { row -> (PublicNotice, Double)? in
            ...
            return (PublicNotice(
                ...
                noticeID: row.string("notice_id"),
                address: row.string("address") ?? "Address unavailable",
                description: plainText(row.string("description")),
                plainLanguage: plainText(row.string("plain_language")),
                conditions: plainText(row.string("conditions")),
                approvalType: row.string("approval_type"),
                decision: row.string("decision"),
                meetingDate: parseDate(row.string("meeting_date")),
                appealDate: parseDate(row.string("appeal_date")),
                community: row.string("community"),
                ward: row.string("ward"),
                neighbourhood: row.string("neighbourhood"),
                rollNumber: row.string("roll_number"),
                reportPlanURL: urlValue(row["report_plan_url"]),
                distanceDescription: distanceDescription(meters: distance),
                coordinate: coordinate
            ), distance)
        }
    ...
    // Shared with domain extensions (e.g. WinnipegProvider+Schools); internal not private.
    func parseCoordinate(_ row: [String: Any]) -> CLLocationCoordinate2D? {
    ...
    private func urlValue(_ value: Any?) -> URL? {
        ...
        if let dict = value as? [String: Any] {
            return dict.string("url").flatMap(URL.init(string:))
        }
        return nil
    }
    ...
    func distanceDescription(from subject: CLLocationCoordinate2D, to coordinate: CLLocationCoordinate2D) -> String {
    ...
    func distanceDescription(meters distanceMeters: Double) -> String {
...
// Internal (not private) so the provider's domain extensions can share these helpers.
extension String {
